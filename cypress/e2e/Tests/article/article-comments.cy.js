...
describe('Article Comments Tests', () => {
  ...
  it.only("Verify the reply comment form functionality.", () => {
    homepage.mainNavigationComponent.getMyAccountOption().click()
    loginPage.login(Cypress.env("email"), Cypress.env("password"))
    myAccountPage.mainNavigationComponent.getBlogOption().click()
    articlesPage.getLatestArticles().eq(0).click()
    articlePage.commentFormComponent.getCommentForm().should('be.visible')
    articlePage.commentsComponent.getReplyButton(0).click()
    articlePage.commentFormComponent.getCommentForm().should('not.exist')
    articlePage.commentsComponent.getReplyFormOfComment(0).should("be.visible")
  })
})
